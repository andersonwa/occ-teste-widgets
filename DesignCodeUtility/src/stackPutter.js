...
function putStackInstanceFile(path, endpoint) {
  ...
  return readMetadata(path, constants.stackInstanceMetadataJson).then(
    (metadata) => {

      if (metadata) {

        return endPointTransceiver[endpoint]([metadata.repositoryId], request().fromPathAs(path, "source").withEtag(metadata.etag)).tap(
