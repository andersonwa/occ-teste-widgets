...
function putThemeFile(path, field) {
  ...
  return readMetadata(path, constants.themeMetadataJson).then(
    (metadata) => {
...
function updateTheme(path, metadata) {
  ...
  return endPointTransceiver.updateThemeSource([metadata.repositoryId], requestBuilder).tap(
    (results) => {
      ...
      if (processPutResult(path, results) && !inTransferMode()) {
        ...
        [themeStylesPath, additionalStylesPath, themeVariablesPath].forEach(
          (themeFilePath) => writeEtag(themeFilePath, results.response.headers.etag))
      }
    })
}
...
function updateThemeSource(path, field, metadata) {
  ...
  return endPointTransceiver.updateThemeSource([metadata.repositoryId], request().fromPathAs(path, field).withEtag(metadata.etag)).tap(
    (results) => {
      ...
      if (processPutResult(path, results)) {
        ...
        [constants.themeVariables, constants.themeStyles, constants.themeAdditionalStyles].forEach(
          (themeFile) => writeEtag(`${dirname(path)}/${themeFile}`, results.response.headers.etag))
      }
    }
  )
}
...
function createAndApplyChangesToTheme(path) {
  ...
  return endPointTransceiver.getThemes("?type=custom").then(
    (results) => {
      ...
      return endPointTransceiver.cloneTheme([results.data.items[0].repositoryId], request().withBody(payload)).then(
        (results) => {
