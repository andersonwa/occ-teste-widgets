...
describe("Endpoint Transceiver", () => {
  ...
  beforeEach((done) => {
    ...
    mockery.mockModules(self, '../utils', '../logger', '../i18n', '../promisingClient', 'os-locale')
  ...
  it("should be able to set up a list of endpoints and then call one of them", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).then(
      () => {
        ...
        self.endPointTransceiver.listOrderTypes().then(
          (results) => {
  ...
  it("should be able to detect when you hit storefront by mistake", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).catch(
      (error) => {
  ...
  it("should be able to do a direct get by full url", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).then(
      () => {
        ...
        self.endPointTransceiver.get("http://localhost:9080/file/v5102134760245801134/widget/v2/quote-widget-multiple/js/quote-tester.js").then(
          (results) => {
  ...
  it("should be able to ensure that we want all resources for all locales", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, true).then(
      () => {
  ...
  it("should be able to specify a target locale", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", "de", false).then(
      () => {
  ...
  it("should be able to detect a non existent locale", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", "silly", false).catch(
      (error) => {
  ...
  it("should be able to support parameter substitution and use of requestBuilder", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).then(
      () => {
        ...
        self.endPointTransceiver.updateWidgetDescriptorJavascript(
          ["rep1234", "something.js"], dummyRequestBuilder).then(
          (results) => {
            expect(self.promisingClientInstance.postAndPromise).toHaveBeenCalledWith("http://localhost:9080/ccadminui/v1/widgetDescriptors/rep1234/javascript/something.js", "dummy request")
            done()
          }
        )
      })
  ...
  it("should be able to support query strings", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).then(
      () => {
        self.endPointTransceiver.getElements("?globals=true", dummyRequestBuilder).tap(
          (results) => {
  ...
  it("should be able to support parameter substitution and query strings", (done) => {
    ...
    self.endPointTransceiver.init("http://localhost:9080", "admin", "admin", null, false).then(
      () => {
        self.endPointTransceiver.updateWidgetDescriptorJavascript(["rep4000", "rep4001"], "?globals=true").tap(
          (results) => {
