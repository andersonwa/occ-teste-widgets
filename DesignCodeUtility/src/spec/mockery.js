...
exports.mock = function (name) {
  ...
  Object.keys(realModule).forEach(
    (key) => {
...
exports.addConvenienceMethods = function(spy) {
   ...
   * @param data
   * @return the data that will be returned by the promise - this can help to keep tests short.
   */
  spy.returnsPromise = function (data) {

    // Construct a promise around the data.
    this.and.callFake(Promise.method(() => data))

    // Return the data in case the caller wants to use it for matching.
    return data
  }
