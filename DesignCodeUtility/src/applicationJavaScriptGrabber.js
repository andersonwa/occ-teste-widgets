...
exports.grabAllApplicationJavaScript = function () {
  ...
  if (endPointTransceiver.serverSupports("listAllApplicationJavaScript", "getApplicationJavaScript")) {
    ...
    return endPointTransceiver.listAllApplicationJavaScript().then(
      (results) => {

        return Promise.each(Object.keys(results.data.items),
          (jsFile) => {

            info("grabbingApplicationJavaScript", {name : jsFile})
            return endPointTransceiver.getApplicationJavaScript([jsFile]).tap(
              (results) => writeFileAndETag(`${constants.globalDir}/${jsFile}`, results.data.source, results.response.headers.etag))
          }
        )
      }
    )
